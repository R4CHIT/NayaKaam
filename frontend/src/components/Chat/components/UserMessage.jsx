...
const UserMessage = ({ messages, currentChat, setSidebarOpen, messagesEndRef, mainBar, setMessages }) => {
  ...
  const MainBar = () => (
    <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center space-x-4">
        ...
        <h3 className="font-semibold text-gray-900 text-lg">{mainBar.username}</h3>
      </div>
      <div className="flex items-center space-x-2">
        {[FiSearch, FiPhone, FiVideo, FiMoreVertical].map((Icon, idx) => (
          <button key={idx} className="p-3 hover:bg-gray-100 rounded-full transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 bg-gray-50">
        <div className="space-y-6">
          {messages.map((msg) => {
            ...
            const profile = isSender ? msg.sender_profile : msg.receiver_profile;

            return (
              <div key={msg.id} className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
                <div className={`flex items-end space-x-3 max-w-md ${!isSender ? " flex-row gap-1 space-x-reverse" : ""}`}>
                  ...
                  <div className={`px-5 py-3 rounded-2xl shadow-sm ${!isSender ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-br-md" : "bg-white text-gray-800 border border-gray-100 rounded-bl-md"}`}>
      ...
      <div className="bg-white border-t border-gray-200 px-6 py-4">
        <div className="flex justify-center h-full items-end space-x-3">
          ...
          <button
            ...
            className={`p-5 relative rounded-full transition-all flex-shrink-0 ${message.trim() ? "text-blue-600" : "bg-white text-gray-400 cursor-not-allowed"}`}
